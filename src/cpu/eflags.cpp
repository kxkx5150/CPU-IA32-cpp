...
int CPU::cpu_get_sf(void)
{
    return (m_lr ^ m_laux) >> 31;
}
void CPU::cpu_set_sf(int set)
{
    m_laux = (m_laux & ~0x80000000) | ((m_lr ^ (set << 31)) & 0x80000000);
}
int CPU::cpu_get_pf(void)
{
    uint32_t v = (m_lr ^ (m_laux & 0x80)) & 0xFF;
    v ^= v >> 4;
    v &= 0x0F;
    return (0x9669 >> v) & 1;
}
void CPU::cpu_set_pf(int set)
{
    m_laux = (m_laux & ~0x80) | (cpu_get_pf() ^ set) << 7;
}
int CPU::cpu_get_oac(void)
{
    uint32_t eflags = (cpu_get_of() * 0x800) | (cpu_get_af() * 0x10) | (cpu_get_cf() * 0x01);
    m_eflags        = (m_eflags & ~(0x800 | 0x10 | 0x01)) | (eflags & (0x800 | 0x10 | 0x01));
    return eflags;
}
int CPU::cpu_get_of(void)
{
    uint32_t lop1;
    switch (m_laux & 63) {
        case MUL:
...
void CPU::cpu_set_zf(int set)
{
    cpu_set_eflags((cpu_get_eflags() & ~0x40) | (set * 0x40));
}
uint32_t CPU::cpu_get_eflags(void)
{
    uint32_t eflags = m_eflags & ~(0x800 | 0x80 | 0x40 | 0x10 | 0x04 | 0x01);
...
void CPU::cpu_set_eflags(uint32_t eflags)
{
