...
int CPU::cpu_set_cpuid(struct cpu_config *x)
{
    m_winnt_limit_cpuid = x->cpuid_limit_winnt;
    return 0;
}
void CPU::cpuid(void)
{
...
int CPU::rdmsr(uint32_t index, uint32_t *high, uint32_t *low)
{
    uint64_t value;
    switch (index) {
        case 0x1B:
...
int CPU::wrmsr(uint32_t index, uint32_t high, uint32_t low)
{
        ...
        if ((m_eflags >> 12 & 3) == 3)
            ...
            if (m_cr[4] & (1 << 0)) {
                ...
                if (!((m_eflags & 0x100000 && temp_flags & (1 << 9)) || (temp_flags & (1 << 8)))) {
                    ...
                    const uint32_t flags_mask = 0xFFFF ^ (0x200 | 0x3000);
...
int CPU::popfd(void)
{
    ...
    if (m_eflags & 0x20000) {
        ...
        if (m_cpl == 0 || (m_cr[0] & 1) == 0) {
            ...
            const uint32_t preserve = 0x100000 | 0x80000 | 0x20000;
            cpu_set_eflags((eflags & ~preserve) | (m_eflags & preserve));
        } else {
            if (cpu_pop32(&eflags))
                return 1;
            eflags            = eflags & ~0x10000;
            uint32_t preserve = 0x3000 | 0x100000 | 0x80000 | 0x20000;
...
int CPU::ltr(uint32_t selector)
{
    ...
    do {
        uint32_t addr_ = tss_addr + 4, shift_ = 2, data_ = tss_desc.raw[1], tag = m_tlb_tags[addr_ >> 12];
...
int CPU::lldt(uint32_t selector)
{
...
uint32_t CPU::lar(uint16_t op1, uint32_t op2)
{
...
uint32_t CPU::lsl(uint16_t op, uint32_t op2)
{
...
void CPU::verify_segment_access(uint16_t sel, int write)
{
...
void CPU::arpl(uint16_t *ptr, uint16_t reg)
{
    reg &= 3;
    if ((*ptr & 3) < reg) {
        *ptr = (*ptr & ~3) | reg;
