...
int CPU::tss_is_16(int type)
{
    return type == BUSY_TSS_286 || type == AVAILABLE_TSS_286;
}
int CPU::load_tss_from_task_gate(uint32_t *seg, struct seg_desc *info)
{
...
int CPU::get_tss_esp(int level, int *dest)
{
    int temp;
    if (tss_is_16(((m_seg_access[6]) & (0x10 | 0x08 | 0x04 | 0x02 | 0x01)))) {
        ...
        do {
            uint32_t addr_ = addr + m_seg_base[6], shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
        ...
        do {
            ...
            if (((addr_ | tag >> shift_) & 3)) {
                ...
                temp = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
    }
    *dest = temp;
    return 0;
}
int CPU::get_tss_ss(int level, int *dest)
{
    int temp;
    if (tss_is_16(((m_seg_access[6]) & (0x10 | 0x08 | 0x04 | 0x02 | 0x01)))) {
...
int CPU::do_task_switch(int sel, struct seg_desc *info, int type, int eip)
{
    ...
    uint32_t tr_base = m_seg_base[6], desc_tbl, old_eflags = cpu_get_eflags();
    ...
    if (type == TASK_JMP || type == TASK_IRET) {
        if (tr_base == (uint32_t)-1)
            util_abort();
        uint32_t segid = ((sel & 4) != 0) ? 8 : 7, addr = m_seg_base[segid] + ((m_seg[6] & ~7)) + 5;
        do {
            ...
            if ((tag >> shift_ & 1)) {
                ...
                desc_tbl = *(uint8_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
        desc_tbl &= ~2;
        do {
            uint32_t addr_ = addr, shift_ = 2, data_ = desc_tbl, tag = m_tlb_tags[addr_ >> 12];
            if ((tag >> shift_ & 1)) {
    ...
    if (tss_type == AVAILABLE_TSS_386 || tss_type == BUSY_TSS_386) {
        do {
            uint32_t addr_ = tr_base + 0x20, shift_ = 2, data_ = eip, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 3)) {
        ...
        do {
            uint32_t addr_ = tr_base + 0x24, shift_ = 2, data_ = old_eflags, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 3)) {
                if (cpu_access_write32(addr_, data_, tag >> 2, 2))
                    return 1;
            } else
        ...
        for (int i = 0; i < 8; i++)
            do {
                uint32_t addr_ = tr_base + 0x28 + (i * 4), shift_ = 2, data_ = m_reg32[i],
        ...
        for (int i = 0; i < 6; i++)
            do {
                uint32_t addr_ = tr_base + 0x48 + (i * 4), shift_ = 2, data_ = m_seg[i], tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 3)) {
                    if (cpu_access_write32(addr_, data_, tag >> 2, 2))
                        return 1;
                } else
        ...
        do {
            uint32_t addr_ = tr_base + 0x0E, shift_ = 2, data_ = eip, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 1)) {
        ...
        do {
            uint32_t addr_ = tr_base + 0x10, shift_ = 2, data_ = old_eflags, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 1)) {
                if (cpu_access_write16(addr_, data_, tag >> 2, 2))
                    return 1;
            } else
        ...
        for (int i = 0; i < 8; i++)
            do {
                uint32_t addr_ = tr_base + 0x12 + (i * 2), shift_ = 2, data_ = m_reg32[i],
                         tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 1)) {
                    if (cpu_access_write16(addr_, data_, tag >> 2, 2))
                        return 1;
        ...
        for (int i = 0; i < 4; i++)
            do {
                uint32_t addr_ = tr_base + 0x22 + (i * 2), shift_ = 2, data_ = m_seg[i], tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 1)) {
                    if (cpu_access_write16(addr_, data_, tag >> 2, 2))
                        return 1;
                } else
    ...
    if (type == TASK_INT || type == TASK_CALL)
        do {
            uint32_t addr_ = tr_base, shift_ = 2, data_ = m_seg[6], tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 1)) {
                if (cpu_access_write16(addr_, data_, tag >> 2, 2))
                    return 1;
            } else
    ...
    uint32_t cr3 = 0, eflags, reg32[8], seg[6], ldt;
    if (tss_type == AVAILABLE_TSS_386 || tss_type == BUSY_TSS_386) {
        do {
            uint32_t addr_ = base + 0x1C, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 3)) {
                if (!cpu_access_read32(addr_, tag >> 0, 0))
                    cr3 = m_read_result;
                else
                ...
                cr3 = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
        do {
            uint32_t addr_ = base + 0x20, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 3)) {
                if (!cpu_access_read32(addr_, tag >> 0, 0))
                    eip = m_read_result;
                else
        ...
        do {
            uint32_t addr_ = base + 0x24, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 3)) {
                if (!cpu_access_read32(addr_, tag >> 0, 0))
                ...
                eflags = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
        for (int i = 0; i < 8; i++)
            do {
                uint32_t addr_ = base + 0x28 + (i * 4), shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 3)) {
                    if (!cpu_access_read32(addr_, tag >> 0, 0))
                    ...
                    reg32[i] = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
            } while (0);
        for (int i = 0; i < 6; i++)
            do {
                uint32_t addr_ = base + 0x48 + (i * 4), shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 1)) {
                    if (!cpu_access_read16(addr_, tag >> 0, 0))
                        seg[i] = m_read_result;
                    else
                    ...
                    seg[i] = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
            } while (0);
        do {
            uint32_t addr_ = base + 0x60, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 3)) {
                if (!cpu_access_read32(addr_, tag >> 0, 0))
                    ldt = m_read_result;
                else
                ...
                ldt = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
    } else {
        do {
            uint32_t addr_ = base + 0x0E, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 1)) {
                if (!cpu_access_read16(addr_, tag >> 0, 0))
                    eip = m_read_result;
                else
        ...
        do {
            uint32_t addr_ = base + 0x10, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 1)) {
                if (!cpu_access_read16(addr_, tag >> 0, 0))
                    eflags = m_read_result;
                else
                ...
                eflags = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
        for (int i = 0; i < 8; i++) {
            do {
                uint32_t addr_ = base + 0x12 + (i * 2), shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 1)) {
                    if (!cpu_access_read16(addr_, tag >> 0, 0))
                        reg32[i] = m_read_result;
                    else
                    ...
                    reg32[i] = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
            } while (0);
            reg32[i] |= 0xFFFF0000;
        }
        for (int i = 0; i < 4; i++)
            do {
                uint32_t addr_ = base + 0x22 + (i * 2), shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 1)) {
                    if (!cpu_access_read16(addr_, tag >> 0, 0))
                        seg[i] = m_read_result;
                    else
        ...
        do {
            uint32_t addr_ = base + 0x2A, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if (((addr_ | tag >> shift_) & 1)) {
                if (!cpu_access_read16(addr_, tag >> 0, 0))
                    ldt = m_read_result;
                else
                ...
                ldt = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
        } while (0);
        seg[4] = 0;
        seg[5] = 0;
    }
    ...
    if (type == TASK_JMP || type == TASK_IRET) {
        if (tr_base == (uint32_t)-1)
            util_abort();
        uint32_t segid = ((sel & 4) != 0) ? 8 : 7, addr = m_seg_base[segid] + ((sel & ~7)) + 5;
        do {
            uint32_t addr_ = addr, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
            if ((tag >> shift_ & 1)) {
                if (!cpu_access_read8(addr_, tag >> 0, 0))
...
int CPU::cpu_interrupt(int vector, int error_code, int type, int eip_to_push)
{
    ...
    if (m_cr[0] & 1) {
        if (m_eflags & 0x20000 && type == INTERRUPT_TYPE_SOFTWARE) {
            if (m_cr[4] & (1 << 0)) {
                ...
                do {
                    uint32_t addr_ = m_seg_base[6] + 0x66, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                    if (((addr_ | tag >> shift_) & 1)) {
                        if (!cpu_access_read16(addr_, tag >> 0, 0))
                        ...
                        redirection_map_index = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                } while (0);
                do {
                    uint32_t addr_ = redirection_map_index - 1 - ((~vector & 0xFF) >> 3) + m_seg_base[6], shift_ = 0,
                             tag = m_tlb_tags[addr_ >> 12];
                    if ((tag >> shift_ & 1)) {
                        if (!cpu_access_read8(addr_, tag >> 0, 0))
                        ...
                        redirection_map_entry = *(uint8_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                } while (0);
                if (!(redirection_map_entry & (1 << (vector & 7)))) {
                    ...
                    do {
                        uint32_t addr_ = vector << 2, shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                        if (((addr_ | tag >> shift_) & 3)) {
                            if (!cpu_access_read32(addr_, tag >> 0, 0))
                            ...
                            idt_entry = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                    } while (0);
                    ...
                    do {
                        ...
                        do {
                            uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = flags_image,
                                     tag = m_tlb_tags[addr_ >> 12];
                            if (((addr_ | tag >> shift_) & 1)) {
                    ...
                    do {
                        STACK_esp = (STACK_esp - 2) & STACK_esp_mask;
                        do {
                            uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = m_seg[1],
                                     tag = m_tlb_tags[addr_ >> 12];
                            if (((addr_ | tag >> shift_) & 1)) {
                                if (cpu_access_write16(addr_, data_, tag >> STACK_mask, STACK_mask))
                                    return 1;
                    ...
                    do {
                        STACK_esp = (STACK_esp - 2) & STACK_esp_mask;
                        do {
                            uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = eip_to_push,
                                     tag = m_tlb_tags[addr_ >> 12];
                            if (((addr_ | tag >> shift_) & 1)) {
                                if (cpu_access_write16(addr_, data_, tag >> STACK_mask, STACK_mask))
                                    return 1;
        ...
        switch (idt_entry_type) {
            case TASK_GATE: {
                ...
                uint16_t tss_selector = cpu_seg_gate_target_segment(&idt_entry), tss_offset = tss_selector & 0xFFFC;
                ...
                if (error_code & 0x10000) {
                    ...
                    if (((tss_access) & (0x10 | 0x08 | 0x04 | 0x02 | 0x01)) == AVAILABLE_TSS_286 ||
                        ...
                        do {
                            STACK_esp = (STACK_esp - 2) & STACK_esp_mask;
                            do {
                                uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = error_code,
                                         tag = m_tlb_tags[addr_ >> 12];
                                if (((addr_ | tag >> shift_) & 1)) {
                                    if (cpu_access_write16(addr_, data_, tag >> STACK_mask, STACK_mask))
                                        return 1;
            ...
            case TRAP_GATE_386: {
                ...
                uint32_t        old_esp = m_reg32[4], old_ss = m_seg[2], esp_mask, ss_base;
                        ...
                        if (idt_entry_type & 8) {
                            if (m_eflags & 0x20000) {
                                do {
                                    ...
                                    do {
                                        uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask,
                            ...
                            do {
                                STACK_esp = (STACK_esp - 4) & STACK_esp_mask;
                                do {
                                    uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = old_ss,
                                             tag = m_tlb_tags[addr_ >> 12];
                                    if (((addr_ | tag >> shift_) & 3)) {
                                        if (cpu_access_write32(addr_, data_, tag >> STACK_mask, STACK_mask))
                                            return 1;
                            ...
                            do {
                                STACK_esp = (STACK_esp - 4) & STACK_esp_mask;
                                do {
                                    uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = old_esp,
                                             tag = m_tlb_tags[addr_ >> 12];
                                    if (((addr_ | tag >> shift_) & 3)) {
                                        if (cpu_access_write32(addr_, data_, tag >> STACK_mask, STACK_mask))
                                            return 1;
                ...
                if (idt_entry_type & 8) {
                    do {
                        STACK_esp = (STACK_esp - 4) & STACK_esp_mask;
                        do {
                            uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = cpu_get_eflags(),
                                     tag = m_tlb_tags[addr_ >> 12];
                            if (((addr_ | tag >> shift_) & 3)) {
                                if (cpu_access_write32(addr_, data_, tag >> STACK_mask, STACK_mask))
                                    return 1;
        ...
        do {
            STACK_esp = (STACK_esp - 2) & STACK_esp_mask;
            do {
                uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = cpu_get_eflags() & 0xFFFF,
                         tag = m_tlb_tags[addr_ >> 12];
                if (((addr_ | tag >> shift_) & 1)) {
                    if (cpu_access_write16(addr_, data_, tag >> STACK_mask, STACK_mask))
                        return 1;
        ...
        m_reg32[4] = (STACK_esp_mask & STACK_esp) | (STACK_original_esp & ~STACK_esp_mask);
        vector <<= 2;
        uint16_t cs = *(uint16_t *)((uint8_t *)m_mem + vector + 2), eip = *(uint16_t *)((uint8_t *)m_mem + vector);
        cpu_load_csip_real(cs, eip);
        m_eflags &= ~(0x200 | 0x100 | 0x40000);
        return 0;
    }
}
void CPU::cpu_exception(int vec, int code)
{
    while (1) {
...
int CPU::jmpf(uint32_t eip, uint32_t cs, uint32_t eip_after)
{
...
uint32_t CPU::call_gate_read_param32(uint32_t addr, uint32_t *dest, int mask)
{
    ...
    do {
        uint32_t addr_ = addr + m_seg_base[2], shift_ = mask, tag = m_tlb_tags[addr_ >> 12];
        if (((addr_ | tag >> shift_) & 3)) {
...
uint16_t CPU::call_gate_read_param16(uint32_t addr, uint32_t *dest, int mask)
{
...
int CPU::callf(uint32_t eip, uint32_t cs, uint32_t oldeip, int is32)
{
    uint32_t STACK_esp, STACK_ss_base, STACK_esp_mask, STACK_mask, STACK_original_esp;
    ;
                ...
                switch (gate_type) {
                    case 0x18 ... 0x1B:
                        if (dpldiff < 0) {
                            ...
                            uint32_t *params          = (uint32_t *)__builtin_alloca(parameter_count * 4);
                            ss_base                   = cpu_seg_get_base(&ss_info);
                            ss_mask                   = ss_access & 0x4000 ? -1 : 0xFFFF;
                            uint32_t old_esp          = m_reg32[4] & m_esp_mask;
                            ...
                            if (cs_type == CALL_GATE_386) {
                                ...
                                do {
                                    STACK_esp = (STACK_esp - 4) & STACK_esp_mask;
                                    do {
                                        uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, data_ = oldeip,
                                                 tag = m_tlb_tags[addr_ >> 12];
                                        if (((addr_ | tag >> shift_) & 3)) {
                                            if (cpu_access_write32(addr_, data_, tag >> STACK_mask, STACK_mask))
                                                return 1;
...
void CPU::iret_handle_seg(int x)
{
    uint16_t access  = m_seg_access[x];
...
int CPU::iret(uint32_t tss_eip, int is32)
{
    uint32_t STACK_esp, STACK_ss_base, STACK_esp_mask, STACK_mask, STACK_original_esp;
    ;
    ...
    if (m_cr[0] & 1) {
        ...
        if (m_eflags & 0x20000) {
            if ((m_eflags >> 12 & 3) == 3) {
                    ...
                    do {
                        do {
                            ...
                            if (((addr_ | tag >> shift_) & 3)) {
                                ...
                                cs = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                        } while (0);
                        STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                    } while (0);
                    do {
                ...
                if (m_cr[4] & (1 << 0)) {
                    ...
                    m_reg32[4] = (STACK_esp_mask & STACK_esp) | (STACK_original_esp & ~STACK_esp_mask);
                    cpu_load_csip_virtual(cs, eip);
                    const uint32_t mask = 0xFFFF ^ (0x3000 | 0x200);
                    if (eflags & 0x200)
                        m_eflags |= 0x80000;
                    else
                        m_eflags &= ~0x80000;
            ...
            if (m_eflags & 0x4000) {
                ...
                do {
                    uint32_t addr_ = m_seg_base[6], shift_ = 0, tag = m_tlb_tags[addr_ >> 12];
                    if (((addr_ | tag >> shift_) & 1)) {
                        if (!cpu_access_read16(addr_, tag >> 0, 0))
                        ...
                        tss_back_link = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                } while (0);
                        ...
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 3)) {
                                    ...
                                    esp = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                        } while (0);
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 3)) {
                                    ...
                                    ss = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                        } while (0);
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 3)) {
                                    ...
                                    es = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                        } while (0);
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 3)) {
                                    ...
                                    ds = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                        } while (0);
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 3)) {
                                    ...
                                    fs = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                        } while (0);
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 3)) {
                                    ...
                                    gs = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                        } while (0);
                        ...
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 1)) {
                                    ...
                                    esp = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 2) & STACK_esp_mask;
                        } while (0);
                        do {
                            do {
                                ...
                                if (((addr_ | tag >> shift_) & 1)) {
                                    ...
                                    ss = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                            } while (0);
                            STACK_esp = (STACK_esp + 2) & STACK_esp_mask;
                        } while (0);
                    }
        ...
        if (is32) {
            do {
                do {
                    uint32_t addr_ = STACK_esp + STACK_ss_base, shift_ = STACK_mask, tag = m_tlb_tags[addr_ >> 12];
                    if (((addr_ | tag >> shift_) & 3)) {
                        if (!cpu_access_read32(addr_, tag >> STACK_mask, STACK_mask))
                            eip = m_read_result;
                        else
...
int CPU::retf(int adjust, int is32)
{
    uint32_t STACK_esp, STACK_ss_base, STACK_esp_mask, STACK_mask, STACK_original_esp;
    ;
    uint32_t eip = 0, cs = 0;
                ...
                do {
                    do {
                        ...
                        if (((addr_ | tag >> shift_) & 3)) {
                            ...
                            new_esp = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                    } while (0);
                    STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                } while (0);
                do {
                    do {
                        ...
                        if (((addr_ | tag >> shift_) & 3)) {
                            ...
                            new_ss = *(uint32_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                    } while (0);
                    STACK_esp = (STACK_esp + 4) & STACK_esp_mask;
                } while (0);
                new_ss &= 0xFFFF;
                ...
                do {
                    do {
                        ...
                        if (((addr_ | tag >> shift_) & 1)) {
                            ...
                            new_esp = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                    } while (0);
                    STACK_esp = (STACK_esp + 2) & STACK_esp_mask;
                } while (0);
                do {
                    do {
                        ...
                        if (((addr_ | tag >> shift_) & 1)) {
                            ...
                            new_ss = *(uint16_t *)((uint8_t *)m_tlb[addr_ >> 12] + addr_);
                    } while (0);
                    STACK_esp = (STACK_esp + 2) & STACK_esp_mask;
                } while (0);
            }
...
void CPU::f_reload_cs_base(void)
{
    uint32_t virt_eip = (m_phys_eip + m_eip_phys_bias);
    uint32_t lin_page = virt_eip >> 12, shift = m_tlb_shift_read, tag = m_tlb_tags[virt_eip >> 12] >> shift;
    if (tag & 2) {
        m_last_phys_eip = m_phys_eip + 0x1000;
        return;
    }
    m_phys_eip      = (uint32_t)(uintptr_t)((uint8_t *)m_tlb[lin_page] + virt_eip - (uint8_t *)m_mem);
    m_last_phys_eip = m_phys_eip & ~0xFFF;
    m_eip_phys_bias = virt_eip - m_phys_eip;
}
int CPU::f_sysenter(void)
{
    uint32_t cs = m_sysenter[0], cs_offset = cs & 0xFFFC;
