...
#define FLOPPY_FATAL(x, ...) FATAL("FLOPPY", x, ##__VA_ARGS__);
...
struct fdc
{
    uint8_t            status[2];
    uint8_t            dor;
    uint8_t            tape_drive[4];
    uint8_t            msr, data_rate;
    uint8_t            fifo;
    uint8_t            dir[4], ccr;
    uint8_t            st[4];
    int                selected_drive;
    int                multi_mode;
    int                format_bytes_to_read, format_byte, format_dma_pos;
...
{
    struct bjson_object *obj = state_obj((char *)"fdc", 36);
    state_field(obj, 2, (char *)"fdc.status", &fdc.status);
    state_field(obj, 1, (char *)"fdc.dor", &fdc.dor);
    state_field(obj, 4, (char *)"fdc.tape_drive", &fdc.tape_drive);
    state_field(obj, 1, (char *)"fdc.msr", &fdc.msr);
    state_field(obj, 1, (char *)"fdc.data_rate", &fdc.data_rate);
    state_field(obj, 1, (char *)"fdc.fifo", &fdc.fifo);
    state_field(obj, 4, (char *)"fdc.dir", &fdc.dir);
    state_field(obj, 1, (char *)"fdc.ccr", &fdc.ccr);
    state_field(obj, 4, (char *)"fdc.st", &fdc.st);
    state_field(obj, 4, (char *)"fdc.selected_drive", &fdc.selected_drive);
    state_field(obj, 4, (char *)"fdc.multi_mode", &fdc.multi_mode);
    state_field(obj, 4, (char *)"fdc.format_bytes_to_read", &fdc.format_bytes_to_read);
    state_field(obj, 4, (char *)"fdc.format_byte", &fdc.format_byte);
    state_field(obj, 4, (char *)"fdc.format_dma_pos", &fdc.format_dma_pos);
    state_field(obj, 16, (char *)"fdc.command_buffer", &fdc.command_buffer);
    state_field(obj, 1, (char *)"fdc.command_buffer_size", &fdc.command_buffer_size);
    state_field(obj, 1, (char *)"fdc.command_buffer_pos", &fdc.command_buffer_pos);
    state_field(obj, 16, (char *)"fdc.response_buffer", &fdc.response_buffer);
    state_field(obj, 1, (char *)"fdc.response_buffer_size", &fdc.response_buffer_size);
    state_field(obj, 1, (char *)"fdc.response_pos", &fdc.response_pos);
    state_field(obj, 8, (char *)"fdc.seek_cylinder", &fdc.seek_cylinder);
    state_field(obj, 8, (char *)"fdc.seek_head", &fdc.seek_head);
    state_field(obj, 8, (char *)"fdc.seek_sector", &fdc.seek_sector);
    state_field(obj, 8, (char *)"fdc.seek_internal_lba", &fdc.seek_internal_lba);
    state_field(obj, 4, (char *)"fdc.interrupt_countdown", &fdc.interrupt_countdown);
    state_field(obj, 16, (char *)"fdc.drive_inserted", &fdc.drive_inserted);
    state_field(obj, 16, (char *)"fdc.drive_size", &fdc.drive_size);
    state_field(obj, 16, (char *)"fdc.drive_heads", &fdc.drive_heads);
    state_field(obj, 16, (char *)"fdc.drive_cylinders", &fdc.drive_cylinders);
    state_field(obj, 16, (char *)"fdc.drive_spt", &fdc.drive_spt);
    state_field(obj, 16, (char *)"fdc.drive_write_protected", &fdc.drive_write_protected);
    state_field(obj, 4, (char *)"fdc.write_length", &fdc.write_length);
    state_field(obj, 1, (char *)"fdc.precomp", &fdc.precomp);
    state_field(obj, 1, (char *)"fdc.config", &fdc.config);
    state_field(obj, 1, (char *)"fdc.locked", &fdc.locked);
    state_field(obj, 1, (char *)"fdc.perpendicular_mode", &fdc.perpendicular_mode);
}
...
static uint32_t fdc_read(uint32_t port)
{
    switch (port) {
        ...
        case 0x3F7: {
            ...
            fdc.dir[fdc.selected_drive] &= ~0x80;
...
static void fdc_write_cb(void *a, int b)
{
    ...
    if (b == 0)
        ...
        fdc_set_st0(SR0_ABNTERM);
    uint32_t drvid = fdc.selected_drive, sector = fdc.seek_sector[drvid], head = fdc.seek_head[drvid],
...
static void fdc_write(uint32_t port, uint32_t data)
{
    ...
    switch (port) {
        ...
        case 0x3F5:
            ...
            if (fdc.command_buffer_pos != fdc.command_buffer_size) {
                ...
                switch (fdc.command_buffer[0]) {
                    case 0x0D:
                    case 0x4D: {
                        int drvid = drvid = fdc.command_buffer[1] & 3, head = fdc.command_buffer[1] >> 2 & 1,
                            sector_size = 128 << fdc.command_buffer[2], sectors = fdc.command_buffer[3],
                    ...
                    case 0xC5:
                    case 0xE5: {
                        int drvid = fdc.command_buffer[1] & 3, head = fdc.command_buffer[1] >> 2 & 1,
                            cylinder = fdc.command_buffer[2], sector = fdc.command_buffer[4],
                            sector_size = fdc.command_buffer[5], max_sector = fdc.command_buffer[6],
                            gap3 = fdc.command_buffer[7], length = fdc.command_buffer[8];
                        UNUSED(gap3);
                        ...
                        if (fdc.command_buffer[0] & 1) {
                            ...
                            int res = drive_read(fdc.drives[drvid], NULL, fdc.dmabuf, bytes_to_read,
                                                 fdc.seek_internal_lba[drvid] << 9, fdc_read_cb);
        ...
        case 0x3f7:
            if (data != 0)
                FLOPPY_FATAL("unknown value written to configuration control register: %02x\n", data);
            break;
        default:
...
static void fdc_handle_format(void *a, int b)
{
    ...
    for (;;) {
        ...
        int res = drive_write(fdc.drives[fdc.selected_drive], NULL, sector, 512,
                              fdc.seek_internal_lba[fdc.selected_drive] << 9, fdc_handle_format);
        if (res == DRIVE_RESULT_ASYNC) {
            fdc.msr = MSR_CB;
            return;
        } else if (res == DRIVE_RESULT_SYNC) {
...
void fdc_dma_complete(void)
{
    switch (fdc.command_buffer[0]) {
        ...
        case 0xC5:
        case 0xE5: {
            int res = drive_write(fdc.drives[fdc.selected_drive], NULL, fdc.dmabuf, fdc.write_length,
                                  fdc.seek_internal_lba[fdc.selected_drive] << 9, fdc_write_cb);
            if (res == DRIVE_RESULT_ASYNC) {
                fdc.msr |= MSR_CB | (fdc.msr & MSR_NDMA ? MSR_DIO | MSR_RQM : 0);
                return;
            } else if (res == DRIVE_RESULT_SYNC) {
...
#define MAKE_DISK_TYPE(h, t, sectors_per_track, type, tdr)                                                             \
    case (h * t * sectors_per_track):                                                                                  \
        ...
        break
            ...
            MAKE_DISK_TYPE(1, 40, 8, 0, 0);
            MAKE_DISK_TYPE(1, 40, 9, 0, 0);
            MAKE_DISK_TYPE(2, 40, 8, 0, 0);
            MAKE_DISK_TYPE(2, 40, 9, 1, 0);
            MAKE_DISK_TYPE(2, 80, 8, 0, 0);
            MAKE_DISK_TYPE(2, 80, 9, 3, 0xC0);
            MAKE_DISK_TYPE(2, 80, 15, 2, 0);
            MAKE_DISK_TYPE(2, 80, 21, 0, 0);
            MAKE_DISK_TYPE(2, 80, 23, 0, 0);
            MAKE_DISK_TYPE(2, 80, 36, 5, 0x40);
            default:
                printf("Unknown disk size: %d, defaulting to 1440K\n", drive->sectors);
                break;
                MAKE_DISK_TYPE(2, 80, 18, 4, 0x80);
        }
    }
